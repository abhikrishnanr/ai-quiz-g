...
const AdminView: React.FC = () => {
  ...
  const ControlButton = ({ 
    ...
    const styles = {
      primary: { active: 'bg-indigo-50 border-indigo-500 shadow-md transform scale-[1.02]', iconActive: 'text-indigo-600', ping: 'bg-indigo-400', dot: 'bg-indigo-500' },
      success: { active: 'bg-emerald-50 border-emerald-500 shadow-md transform scale-[1.02]', iconActive: 'text-emerald-600', ping: 'bg-emerald-400', dot: 'bg-emerald-500' },
      danger: { active: 'bg-red-50 border-red-500 shadow-md transform scale-[1.02]', iconActive: 'text-red-600', ping: 'bg-red-400', dot: 'bg-red-500' }
    };
          ...
          <div className="lg:col-span-3 space-y-4 h-fit">
                       ...
                       <div className="flex gap-1.5">
                         ...
                         <span className={`text-[9px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded border ${
                            session.currentQuestionId === q.id 
                              ...
                              : `border-${getDifficultyColor(q.difficulty)}-200 text-${getDifficultyColor(q.difficulty)}-600`
                         }`}>
                            {q.difficulty}
                         </span>
                       </div>
            ...
            >
               ...
               <div className="flex items-center gap-3 mb-2">
                 ...
                 <span className="text-xs font-black uppercase tracking-widest text-indigo-200">Bodhini AI (Gemini)</span>
               </div>
              ...
              <div className="bg-slate-50/50 p-6 grid grid-cols-2 md:grid-cols-4 gap-3 border-b border-slate-100">
                ...
                <button
                  ...
                  className={`flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all duration-200 h-28 group ${
                    session.status === QuizStatus.REVEALED ? 'bg-indigo-600 border-indigo-600 shadow-md' : 'bg-white border-slate-200 hover:border-indigo-300 hover:bg-slate-50 opacity-100 disabled:opacity-50 disabled:cursor-not-allowed'
                  }`}
                >
              ...
              <div className="p-8 min-h-[400px] flex flex-col">
                {currentQuestion ? (
                  <div className="flex-grow space-y-8 animate-in fade-in slide-in-from-bottom-4">
                    ...
                    <div>
                      <h2 className="text-3xl md:text-4xl font-black text-slate-900 leading-tight mb-4">{currentQuestion.text}</h2>
                      <div className="bg-slate-100 p-4 rounded-xl border border-slate-200 flex gap-3 items-start">
                        ...
                        <div>
                          <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Bodhini Hint Payload</p>
                          <p className="text-sm font-medium text-slate-600 italic">"{currentQuestion.hint}"</p>
                        </div>
                      </div>
                    </div>
                    <div className="grid gap-3">
                      {currentQuestion.options.map((opt, i) => {
                         ...
                         return (
                           <div key={i} className={`flex items-center gap-4 p-4 rounded-xl border-2 transition-all ${isCorrect ? 'bg-emerald-50 border-emerald-200 shadow-sm' : 'bg-white border-slate-100 opacity-60 grayscale-[0.5]'}`}>
                              <div className={`w-8 h-8 rounded-lg flex items-center justify-center font-black text-lg ${isCorrect ? 'bg-emerald-500 text-white' : 'bg-slate-200 text-slate-500'}`}>
